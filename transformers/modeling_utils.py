...
@contextmanager   # 加上这个就可以用with 语句了.
def no_init_weights(_enable=True):
    ...
    if _enable:
        _init_weights = False
    try:        # 定义一个全局变量开始让他为False, 然后运行with里面代码.运行完让他变成true. 这么写可以避免并发时候发生错误.当然我们一般也不会让python写并发.
        yield
    finally:
        _init_weights = True
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        # Load config if we don't provide a configuration # 如果没给配置文件,就按照modelname来自动加载.
        if not isinstance(config, PretrainedConfig):
        ...
        if is_deepspeed_zero3_enabled():
            ...
            with no_init_weights(_enable=_fast_init): # 使用config初始化模型. 搭建网络.
                model = cls(config, *model_args, **model_kwargs)
        ...
        if from_tf:
            if resolved_archive_file.endswith(".index"):
                ...
                try:
                    ...
                    raise
        elif from_flax:  # flax也是一种框架的model模型. 跟tf一样.我们不用.
            try:
            ...
            if state_dict is None: # 如果没有自定义model权重.我们就加载官方权重.
                try:
