...
class DetrFrozenBatchNorm2d(nn.Module):
    ...
    def __init__(self, n):
        ...
        self.register_buffer("running_mean", torch.zeros(n))       # 跟 等号赋值一样.
    ...
    def forward(self, x):
        ...
        epsilon = 1e-5
        scale = weight * (running_var + epsilon).rsqrt()  #rsqrt 平方根的倒数.  var 是方差
        bias = bias - running_mean * scale  #  减平均数
        return x * scale + bias
...
# 特征输出4个,  返回网络的1,2,3,4 层结果. 第一层.特征256 ,然后512 ,1024, 2048
        out = []
        for feature_map in features:
            # downsample pixel_mask to match shape of corresponding feature_map
            mask = nn.functional.interpolate(pixel_mask[None].float(), size=feature_map.shape[-2:]).to(torch.bool)[0]  # 使用插值计算mask
...
class DetrConvModel(nn.Module):
    ...
    def forward(self, pixel_values, pixel_mask):
        ...
        out = self.conv_encoder(pixel_values, pixel_mask)  # out:   1,256,200,267 mask  1, 200,267
        pos = []
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    return inverted_mask.masked_fill(inverted_mask.bool(), torch.finfo(dtype).min) # 用负无穷来填补1
...
class DetrSinePositionEmbedding(nn.Module):
    ...
    def forward(self, pixel_values, pixel_mask):
        ...
        x_embed = pixel_mask.cumsum(2, dtype=torch.float32) # x,y位置索引
        if self.normalize:
            y_embed = y_embed / (y_embed[:, -1:, :] + 1e-6) * self.scale # 归一化,除以最大值而已
            x_embed = x_embed / (x_embed[:, :, -1:] + 1e-6) * self.scale
# 打入的维度是256. 这个算法还是用的绝对位置编码.给每一个pixel定义一个位置编码. 算法跟原始transformers是一样的. 只不过区别是从nlp的一维变成了cv里面的二维. 所以最后的 物理含义是pos: batchsize  dimension  width height  其中第二个维度是256, 256 是 表示pixel 的横纵坐标. 每一个给128维度表示!!!!!!!!!没啥难度.
...
class DetrAttention(nn.Module):
    ...
    def forward(
        ...
        if position_embeddings is not None:  # 这个位置编码直接就是 linear层出来的.
            hidden_states_original = hidden_states
            hidden_states = self.with_pos_embed(hidden_states, position_embeddings) #  加上位置信息.
...
# query 代表的是target, key 代表的是 src
        ...
        if attention_mask is not None:
            ...
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len) # 因为加法可能触发shape补齐.
...
class DetrEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, attn_weights = self.self_attn(  # 跟nlp一样也是自注意力.
        ...
        if torch.isinf(hidden_states).any() or torch.isnan(hidden_states).any():
            clamp_value = torch.finfo(hidden_states.dtype).max - 1000  # 读取数据类型的最大值.
...
class DetrEncoder(DetrPreTrainedModel):
    ...
    def forward(
        ...
        for i, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer# droplayer只有在测试的时候才会进行, 测试的时候这些东西直接跳过.也就是每一层都要.不尽兴drop ,这里面我们使用的参数是0.
...
class DetrDecoder(DetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states=None,
        return_dict=None,
    ): # 这个部分就是网络部分的核心和复杂点了. 思路是transfromer的翻译任务. 输入一个全0的query矩阵,然后跟encoder_hidden通过注意力,翻译出来一堆分类信息和框. 当然只是一步出来结果,而不像nlp里面seq迭代出来.
        r"""
        Args:
            inputs_embeds (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`):
                The query embeddings that are passed into the decoder.=========全0
            ...
            position_embeddings (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):  #加入qery和key 体现位置信息.
                Position embeddings that are added to the queries and keys in each cross-attention layer.
            query_position_embeddings (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, num_queries, hidden_size)`):, `optional`): # 重要.
                Position embeddings that are added to the queries and keys in each self-attention layer.
            output_attentions (:obj:`bool`, `optional`):
                Whether or not to return the attentions tensors of all attention layers. See ``attentions`` under
                returned tensors for more detail.
        ...
        # optional intermediate hidden states  # 是否使用中间层来计算loss.
...
class DetrModel(DetrPreTrainedModel):
    ...
    def forward(
        ...
        if pixel_mask is None: # 如果之前忘了给,就默认值都不进行maks.全是1.
            pixel_mask = torch.ones(((batch_size, height, width)), device=device)

        # First, sent pixel_values + pixel_mask through Backbone to obtain the features
        # pixel_values should be of shape (batch_size, num_channels, height, width)
        # pixel_mask should be of shape (batch_size, height, width)
        features, position_embeddings_list = self.backbone(pixel_values, pixel_mask)#  先用conv提取特征.

        # get final feature map and downsampled mask
        feature_map, mask = features[-1]  # 我们只用了最后一个特征.
        ...
        # In other words, turn their shape into (batch_size, sequence_length, hidden_size)  # 形变即可.
...
#===============下面就是transformer网络
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                attention_mask=flattened_mask,  # 图像里面不存在因果,所以mask就是pad影响.
                position_embeddings=position_embeddings,
                output_attentions=output_attentions,
...
class DetrForObjectDetection(DetrPreTrainedModel):
    def __init__(self, config: DetrConfig):
        ...
        self.class_labels_classifier = nn.Linear(
            config.d_model, config.num_labels + 1  # 因为coco没有过空物体类别所以我们最后加一类.
    ...
    def forward(
        ...
        sequence_output = outputs[0]  # 1, 100, 256  100表示query数量.表示100个结果.
