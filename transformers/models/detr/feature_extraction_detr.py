...
class DetrFeatureExtractor(FeatureExtractionMixin, ImageFeatureExtractionMixin):
    ...
    def prepare_coco_detection(self, image, target, return_segmentation_masks=False):
        ...
        anno = target["annotations"]  # 一张图片对应 一个标注组成的数组叫anno
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            ...
            annotations (:obj:`Dict`, :obj:`List[Dict]`, `optional`):  # 看看标注数据集, 做训练.
        ...
        if annotations is not None:
            if not is_batched:
                if self.format == "coco_detection": # 用于 目标识别
                    if isinstance(annotations, dict) and "image_id" in annotations and "annotations" in annotations:
                        if isinstance(annotations["annotations"], (list, tuple)):
                            # an image can have no annotations  非batch图片不让train
                            if len(annotations["annotations"]) == 0 or isinstance(annotations["annotations"][0], dict):
                                valid_annotations = True
                elif self.format == "coco_panoptic": # 用于 segmentation
        ...
        if self.do_resize and self.size is not None: # 把输入图片进行resize 到最短边800
            if annotations is not None:
                for idx, (image, target) in enumerate(zip(images, annotations)):
        ...
        if pad_and_return_pixel_mask: # 是否进行batch内pad到最大图片. 果断开启啊!
            ...
            for image in images:
                # create padded image
                padded_image = np.zeros((c, h, w), dtype=np.float32) # 来个空白的大的,然后内部填上图片.
                padded_image[: image.shape[0], : image.shape[1], : image.shape[2]] = np.copy(image)# 填补方式是右下补0.
